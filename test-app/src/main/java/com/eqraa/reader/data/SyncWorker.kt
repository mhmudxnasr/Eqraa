...
import com.eqraa.reader.data.HighlightDto
...
class SyncWorker(
    ...
    private suspend fun processAction(action: SyncAction): Boolean {
        return when (action.type) {
            SyncAction.TYPE_POSITION -> {
                ...
                val response = supabase.postgrest.rpc("upsert_reading_progress", params).decodeAs<com.eqraa.reader.data.model.SyncResponseDto>()
                if (response.updated) {
    ...
    private suspend fun handleFailedAction(
        syncDao: com.eqraa.reader.data.db.SyncDao,
        action: SyncAction
    ) {
        val maxRetries = 10
