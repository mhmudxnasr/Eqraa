...
import com.eqraa.reader.data.model.CloudBookDto
import com.eqraa.reader.data.model.Book
import com.eqraa.reader.databinding.FragmentBookshelfBinding
import com.eqraa.reader.opds.GridAutoFitLayoutManager
...
import com.eqraa.reader.ui.sync.ConflictResolutionDialog
import com.eqraa.reader.data.auth.AuthRepository
import com.eqraa.reader.data.model.SyncConflict
import android.graphics.Color
import android.content.res.ColorStateList
import android.widget.ImageView
import com.eqraa.reader.ui.sync.SyncStatusViewModel.SyncStatus
                ...
                try {
                    ...
                    val jobs = listOf(
                        launch { 
                            ...
                            com.eqraa.reader.data.SyncWorker.enqueue(requireContext())
                        },
        ...
        checkConnectionBtn.setOnClickListener {
            lifecycleScope.launch {
                ...
                val (title, message) = when (result) {
                    is com.eqraa.reader.data.CloudLibraryManager.ConnectionResult.Success ->
                        "Success" to "All systems operational.\n- Server Reachable\n- Auth Valid\n- Database Connected"
                    is com.eqraa.reader.data.CloudLibraryManager.ConnectionResult.ServerUnreachable ->
                        "Server Unreachable" to result.message
                    is com.eqraa.reader.data.CloudLibraryManager.ConnectionResult.AuthError ->
                        "Authentication Failed" to "Server returned ${result.code}. Check your API token."
                    is com.eqraa.reader.data.CloudLibraryManager.ConnectionResult.DatabaseError ->
                        "Database Error" to result.message
                    is com.eqraa.reader.data.CloudLibraryManager.ConnectionResult.UnknownError ->
    ...
    private fun toggleBookSync(book: Book) {
        lifecycleScope.launch {
            ...
            // Using App.bookRepository seems best.
                val dao = com.eqraa.reader.data.db.AppDatabase.getDatabase(requireContext()).booksDao()
                dao.insertBook(book)
